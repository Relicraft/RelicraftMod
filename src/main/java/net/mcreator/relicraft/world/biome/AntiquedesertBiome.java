...
package net.mcreator.relicraft.world.biome;
...
public class AntiquedesertBiome extends RelicraftModElements.ModElement {
	public static Biome biome;
	public AntiquedesertBiome(RelicraftModElements instance) {
		super(instance, 22);
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-52).setWaterColor(4159204).setWaterFogColor(329011).withSkyColor(-52)
						.withFoliageColor(10387789).withGrassColor(-3342541).build();
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder()
						.withSurfaceBuilder(SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(AntiqueSandBlock.block.getDefaultState(),
								AntiqueSandBlock.block.getDefaultState(), AntiqueSandBlock.block.getDefaultState())));
				biomeGenerationSettings.withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.TREE
						.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(DryCactusBlock.block.getDefaultState()),
								new SimpleBlockStateProvider(Blocks.AIR.getDefaultState()),
								new BlobFoliagePlacer(FeatureSpread.func_242252_a(2), FeatureSpread.func_242252_a(0), 3),
								new StraightTrunkPlacer(3, 2, 0), new TwoLayerFeature(1, 0, 1))).setIgnoreVines().setMaxWaterDepth(0).build())
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.NONE).category(Biome.Category.DESERT).depth(0.1f).scale(0.2f)
						.temperature(1.8f).downfall(0f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("relicraft:antiquedesert"));
			}
		}
	}
	@Override
